...
def run_pipeline(shared_dir: str, video_name: str) -> None:
    print("Stage 1 — ffmpeg0: Extract full audio")
    ffmpeg0.run(shared_dir, video_name)

    print("Stage 2 — librosa: Detect silence / find cut points")
    librosa_stage.run(shared_dir, video_name)

    print("Stage 3 — ffmpeg1: Segment video into clips")
    ffmpeg1.run(shared_dir, video_name)

    print("Stage 4 — ffmpeg2: Per-clip audio extraction")
    ffmpeg2.run(shared_dir, video_name)

    print("Stage 5 — Whisper: Speech-to-text transcription")
    whisper_asr.run(shared_dir, video_name)

    print("Stage 6 — ffmpeg3: Key frame extraction")
    ffmpeg3.run(shared_dir, video_name)

    print("Stage 7 — object_detector: YOLOv4 detection")
